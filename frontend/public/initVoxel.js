// Advanced renderer: collision, human models (GLTF preferred), hiding-spot markers, shooter markers.
// Exposes VoxelWorld API with collision tests and helper functions used by game.js.
...
(async function(){
  ...
  const blockSolid = new Set([BLOCK_GRASS, BLOCK_DIRT, BLOCK_STONE, BLOCK_WOOD, BLOCK_BUILDING, BLOCK_ROAD, BLOCK_BUSH]);
  const blockColors = {
  ...
  let playerEntity, playerPos = { x:0, y:0, z:0, crouch:false }, yaw=0, pitch=0;
  const chunks = {}; // key -> { blocks: Int8Array, group }
  const entitiesGroup = new THREE.Group();
  const remote = {}; // id -> { ent, target, labelCanvas }
  let humanModel = null;

  // try load GLTF human model (optional)
  async function loadHumanModel() {
    try {
      ...
      return await new Promise((res, rej) => loader.load('/assets/human.glb', res, null, rej));
    } catch(e) { return null; }
  }

  function overlayMessage(text){ let o=document.getElementById('voxelErrorOverlay'); if(!o){ o=document.createElement('div'); o.id='voxelErrorOverlay'; Object.assign(o.style,{position:'fixed',inset:'0',display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.6)',color:'#fff',zIndex:10000,fontSize:'18px',padding:'20px'}); document.body.appendChild(o);} o.textContent=text; o.style.display='flex'; }
  function hideOverlay(){ const o=document.getElementById('voxelErrorOverlay'); if(o) o.style.display='none'; }
  function canWebGL(){ try{ const c=document.createElement('canvas'); return !!(window.WebGLRenderingContext && (c.getContext('webgl')||c.getContext('experimental-webgl'))); }catch(e){return false;} }

  function initRenderer(){
    if (typeof THREE === 'undefined') throw new Error('three.js missing');
    if (!canWebGL()) throw new Error('WebGL not available');
  ...
  function createLabelCanvas(name){
    ...
    ctx.fillStyle='rgba(255,255,255,0.95)'; ctx.font='20px sans-serif'; ctx.textAlign='center'; ctx.fillText(name, c.width/2, 42); return c;
  }

  function createHumanoid(colorHex, name) {
    if (humanModel) {
      const clone = humanModel.scene ? humanModel.scene.clone(true) : humanModel.clone(true);
      const spr = new THREE.Sprite(new THREE.SpriteMaterial({ map: new THREE.CanvasTexture(createLabelCanvas(name)), depthTest:false, depthWrite:false }));
      spr.scale.set(2.6,0.8,1); spr.position.set(0,2.8,0); clone.add(spr); clone.userData._labelCanvas = spr.material.map.image; return clone;
    }
    ...
    const legL = new THREE.Mesh(new THREE.CylinderGeometry(0.18,0.18,1), matCloth); legL.position.set(-0.18,0.5,0);
    ...
    const spr = new THREE.Sprite(new THREE.SpriteMaterial({ map: new THREE.CanvasTexture(createLabelCanvas(name)), depthTest:false, depthWrite:false }));
    spr.scale.set(2.6,0.8,1); spr.position.set(0,3.0,0); g.add(spr); g.userData._labelCanvas = spr.material.map.image;
    return g;
  }
  ...
  async function initScene(){
    ...
    humanModel = await loadHumanModel().catch(()=>null);
    playerEntity = createHumanoid(0xffcc66, (window.sessionStorage && JSON.parse(sessionStorage.getItem('survive.session.v1')||'{}').name) || 'You');
  ...
  async function requestChunk(cx,cz){
    ...
    } catch(e){ console.warn('requestChunk failed', e); }
  }
  ...
  let lastShooterMarkers = [];
  function showShooterMarker(shooterPos, shooterId, duration=4000){
    lastShooterMarkers.forEach(m=>scene.remove(m)); lastShooterMarkers = [];
    const s = new THREE.Mesh(new THREE.TorusGeometry(0.45,0.08,8,12), new THREE.MeshStandardMaterial({ color:0xff4444, emissive:0xff4444 }));
    s.position.set(shooterPos.x, shooterPos.y+1.6, shooterPos.z);
    scene.add(s); lastShooterMarkers.push(s);
    const start = new THREE.Vector3(playerPos.x, playerPos.y+1.6, playerPos.z);
    ...
    scene.add(line); lastShooterMarkers.push(line);
    setTimeout(()=> { lastShooterMarkers.forEach(m=>scene.remove(m)); lastShooterMarkers = []; }, duration);
  }
  ...
  function setupControls(){
    ...
    function update(dt){
      ...
      const phase = window._myPhaseActive; // set by game.js when phase active
      if (phase || isPositionFree(nx, nz)) {
        playerPos.x = nx; playerPos.z = nz; playerEntity.position.set(playerPos.x, playerEntity.position.y, playerPos.z);
      }
  ...
  function ensureRemote(id, info){
    if (!remote[id]) {
      const ent = createHumanoid(info.role === 'seeker' ? 0xff6666 : 0x99ff99, info.name || 'Player');
  ...
  function updatePlayers(list){
    ...
    for (const p of list) {
      ...
      if (r.ent.userData && r.ent.userData._labelCanvas) {
    ...
    for (const id of Object.keys(remote)) if (!ids.has(id)) { scene.remove(remote[id].ent); delete remote[id]; }
  }
  ...
  window.VoxelWorld = {
    start: function(){ try{ initRenderer(); initScene(); setupControls(); return true; } catch(e){ console.error('VoxelWorld start failed', e); overlayMessage('Graphics init failed: '+e.message); return false; } },
    requestChunk,
  ...
  console.info('[world] advanced renderer ready');
})();
